...
plot(input/2/pi,MagdB_e,"black",'linewidth',2); hold on;
plot(input/2/pi,MagdB2_e,'r--','linewidth',2); grid on;
ylim([-15,0]);
ylabel('\textbf{Gain~(dB)}','fontsize',12,'interpreter','latex');
hAxis(3)=subplot(3,2,3);
plot(input/2/pi,Phase_e,"black",'linewidth',2); hold on;
plot(input/2/pi,Phase2_e,'r--','linewidth',2); grid on;
ylim([-3,0]);
ylabel('\textbf{Phase~(Rad)}','fontsize',12,'interpreter','latex');
hAxis(5)=subplot(3,2,5);
plot(input/2/pi,Time_delay_e,"black",'linewidth',2); hold on;
plot(input/2/pi,Time_delay2_e,'r--','linewidth',2); grid on;
ylim([-30,-10]);
...
area2=fill([input;input(N:-1:1)]/2/pi,[MagdB_u;MagdB_l(N:-1:1)],"black"); hold on; 
set(area2,'LineStyle','none');     set(area2,'facealpha',1)
area1=fill([input;input(N:-1:1)]/2/pi,[MagdB2_u;MagdB2_l(N:-1:1)],'r'); grid on;
set(area1,'LineStyle','none');     set(area1,'facealpha',0.5);
ylim([-15,0]);
hAxis(4)=subplot(3,2,4);
area2=fill([input;input(N:-1:1)]/2/pi,[Phase_u;Phase_l(N:-1:1)],"black"); hold on; 
set(area2,'LineStyle','none');     set(area2,'facealpha',1)
area1=fill([input;input(N:-1:1)]/2/pi,[Phase2_u;Phase2_l(N:-1:1)],'r'); grid on;
set(area1,'LineStyle','none');     set(area1,'facealpha',0.5);
ylim([-3,0]);
hAxis(6)=subplot(3,2,6);
area2=fill([input;input(N:-1:1)]/2/pi,[Time_delay_u;Time_delay_l(N:-1:1)],"black"); hold on; 
set(area2,'LineStyle','none');     set(area2,'facealpha',0.5)
area1=fill([input;input(N:-1:1)]/2/pi,[Time_delay2_u;Time_delay2_l(N:-1:1)],'r'); grid on;
set(area1,'LineStyle','none');     set(area1,'facealpha',0.5);
ylim([-30,-10]);
xlabel('\textbf{Frequency~(Hz)}','fontsize',12,'interpreter','latex');
