...
plot(t,Phi,'b','linewidth',2); grid on;
...
for i=1:2
    ...
    ylabel('Compact error (m/s)','fontsize',12,'interpreter','latex');
end
...
legend({'{\it E}-trajectory'},'fontsize',12,'interpreter','latex','location','NorthEast');
title('{\it E}-trajectory and time-varying boundary layer','fontsize',12,'interpreter','latex');
% print -depsc F_E_trajectory;

%%
figure; set(gcf,'Position',[0 0 900 450]);
subplot(2,2,1:2);
plot(t,u0,'k','linewidth',1); hold on;
plot(t,u,'--b','linewidth',1); grid on;
axesH = gca;
set(axesH,'fontsize',16,'TickLabelInterpreter','latex');
%axesH.XAxis.TickLabelFormat ='\\textbf{%g';axesH.YAxis.TickLabelFormat ='\\textbf{%g';
ylabel('Adjusted command','fontsize',12,'interpreter','latex');
xlabel('Time (s)','fontsize',12,'interpreter','latex');
xlim([0,40]);
ylim([-240,240]); set(gca,'YTick',-240:120:240);
legend({'Without boundary layer','With boundary layer'},'fontsize',12,'interpreter','latex','location','NorthEast');
subplot(2,2,3);
plot(t,u0,'k','linewidth',1); hold on;
...
figure; set(gcf,'Position',[0 0 900 450]);
subplot(2,1,1);
plot(t,y_gc0,"black",'linewidth',1); hold on;
plot(t,y_gc,'--c','linewidth',2); grid on;
xlim([0,40]); set(gca,'XTick',[0 5 7 8 10 20 30 40]);
ylim([-1,1]); set(gca,'YTick',-1:0.5:1);
line([7,7],[-1,1],'linestyle','-','Color','k','linewidth',1.5);
line([8,8],[-1,1],'linestyle','-','Color','k','linewidth',1.5);
legend({'Without boundary layer','With boundary layer'},'fontsize',12,'interpreter','latex','location','NorthEast');
subplot(2,1,2);
plot(t,y_gc,'--b','linewidth',1); grid on;
xlim([0,40]); set(gca,'XTick',[0 5 7 8 10 20 30 40]);
ylim([-1e-2,1e-2]); set(gca,'YTick',-1e-2:0.5e-2:1e-2);
line([7,7],[-1,1],'linestyle','-','Color','k','linewidth',1.5);
line([8,8],[-1,1],'linestyle','-','Color','k','linewidth',1.5);
legend({'With boundary layer'},'fontsize',12,'interpreter','latex','location','NorthEast');
...
for i=1:2
    ...
    set(axesH,'fontsize',16,'TickLabelInterpreter','latex');
    %axesH.XAxis.TickLabelFormat ='\\textbf{%g';axesH.YAxis.TickLabelFormat ='\\textbf{%g';
    ylabel('Actual command','fontsize',12,'interpreter','latex');
    xlabel('Time (s)','fontsize',12,'interpreter','latex');
end
...
set(gca,'ytick',[]);
set(gca,'xtick',[]);
annotation('arrow',[0.32 0.29],[0.66 0.66],'linestyle','-','Color','k','linewidth',1.5);
...
set(gca,'ytick',[]);
set(gca,'xtick',[]);
annotation('arrow',[0.32 0.29],[0.18 0.18],'linestyle','-','Color','k','linewidth',1.5);
...
for i=1:3
    subplot(3,1,i);
    plot(t,Ref_Resp(:,4-i)*1000,"blue",'linewidth',2); hold on;
    plot(t,RTHS_PI(:,4-i)*1000,'--r','linewidth',1.5); grid on;
    plot(t,RTHS_SMC(:,4-i)*1000,':k','linewidth',2);
    xlim([6,20]);
    axesH = gca;
    set(axesH,'fontsize',16,'TickLabelInterpreter','latex');
    %axesH.XAxis.TickLabelFormat ='\\textbf{%g';axesH.YAxis.TickLabelFormat ='\\textbf{%g';
...
ylabel('$3^{rd}$ floor (mm)','fontsize',12,'interpreter','latex');
subplot(3,1,2);
set(gca,'YTick',-10:5:10);
ylabel('$2^{nd}$ floor (mm)','fontsize',12,'interpreter','latex');
subplot(3,1,3);
ylim([-6,6]);set(gca,'YTick',-6:3:6);
ylabel('$1^{st}$ floor (mm)','fontsize',12,'interpreter','latex');
xlabel('Time (s)','fontsize',16,'interpreter','latex');
legend({'Reference model','PI-P','SMC'},...
    'fontsize',12,'interpreter','latex','location','Northeast','orientation','horizontal');
